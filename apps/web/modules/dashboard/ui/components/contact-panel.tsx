...
export const ContactPanel = () => {
  ...
  const parseUserAgent = useMemo(() => {
    return (userAgent?: string) => {
      ...
      return {
        browser: result.browser.name || "Tidak diketahui",
        browserVersion: result.browser.version || "",
        os: result.os.name || "Tidak diketahui",
        osVersion: result.os.version || "",
        device: result.platform.type || "dekstop",
    ...
    return [
      {
        ...
        items: [
          ...
          {
            label: "Screen",
            values: contactSession.metadata.screenResolution,
          },
          {
            label: "Viewport",
            values: contactSession.metadata.viewportSize,
          },
          {
            label: "Cookies",
            values: contactSession.metadata.cookieEnable ? "Enabled" : "Disabled",
          },
        ],
      },
      {
        ...
        items: [
          ...(countryInfo
            ? [
                {
                  label: "Country",
                  values: <span>{countryInfo.name}</span>,
                },
              ]
            : []),
          {
            label: "Language",
            values: contactSession.metadata.language,
          },
          {
            label: "Timezone",
            values: contactSession.metadata.timezone,
          },
        ],
      },
      {
        ...
        items: [
          {
            ...
            values: new Date(contactSession._creationTime).toLocaleString(),
          },
        ],
      },
    ];
                  ...
                  <div className="space-y-2 text-sm">
                    {section.items.map((item) => (
                      <div className="flex justify-between" key={`${section.id}-${item.label}`}>
                        ...
                        <span className={item.className}>{item.values}</span>
                      </div>
                    ))}
                  </div>
                </AccordionContent>
